<?php include 'includes/header.php' ?>
	...
	<div id="container">
		...
		<div id="left-nav2" style="margin-top:20px;">
			<div class="clip2">
    ...
    <?php
    	include("includes/database.php");
    			...
    			while($row=mysqli_fetch_array($query)){
    				$id = $row['user_id'];
    ?>
    		<div id="left-nav1" style="margin-top:200px;">
    			<h2>Personal Info</h2>
    			<table>
    				<tr>
...
<?php
	include("includes/database.php");
		$q = mysqli_query($conn, "SELECT * from transaction t WHERE t.from_id = '$id' ORDER BY t.id DESC");
		if($q->num_rows > 0){
		while($r=mysqli_fetch_array($q)){
			$pid = $r['post_id'];
			$query=mysqli_query($conn, "SELECT * from post LEFT JOIN user on user.user_id = post.user_id WHERE post.post_id='$pid'");
			while($row=mysqli_fetch_array($query)){
        ...
        $post_user_id = $row['user_id'];
				$time=$row['created'];
?>
		<div id="right-nav1" style="margin-top:20px;">
      ...
      <?php } ?>
		...
		<center>
      <a href="<?php echo $location1; ?>" class="btn btn-sign-up" download style="color: #fff; background-color: #0bacff; border-radius: 10px; margin-top:10px; margin-bottom:10px;">
          <i class="fa fa-download" style="font-size: 20px;"></i> Download file
      </a>
		</center>
		</div>
...
?>
	...
	<div id="right-nav" style="text-align:center; margin-top:18%;">
